...
static clipnode_t *C_NewRange(binangle stAng, binangle endAng)
{
	...
	for (i = 0; i < MAXCLIPNODES; i++)
	{
		if (!clipnodes[i].used)
			break;
	}
	if (i == MAXCLIPNODES - 1)
		I_Error("C_NewRange: Out of clipnodes (max %d).\n", MAXCLIPNODES);
	// Initialize the node.
	cnode = clipnodes + i;
	cnode->used = 1;
	cnode->start = stAng;
...
{
	if (!crange->used)
		I_Error("Tried to remove an unused range.\n");
...
static void C_AddRange(binangle startAngle, binangle endAngle)
{
	...
	OGL_DEBUG("=== C_AddRange(%x,%x):\n", startAngle, endAngle);
	if (startAngle == endAngle)
	{
		OGL_DEBUG("  range has no length, skipping\n");
		return;
	}
	...
	for (ci = cliphead; ci; ci = ci->next)
	{
		...
		{
			OGL_DEBUG("  range already exists\n");
	...
	for (ci = cliphead; ci; )
	{
		...
		{
			crange = ci;
			OGL_DEBUG("  removing contained range %x => %x\n", crange->start, crange->end);
	...
	if (!cliphead)
	{
		...
		OGL_DEBUG("  new head node added, %x => %x\n", cliphead->start, cliphead->end);
		return;
	}
	...
	for (ci = cliphead; ci; ci = ci->next)
	{
		...
		if (ci->start >= startAngle && ci->start <= endAngle)
		{
			...
			OGL_DEBUG("  overlapping beginning with %x => %x, ", ci->start, ci->end);
			ci->start = startAngle;
			OGL_DEBUG("adjusting ci to %x => %x\n", ci->start, ci->end);
			return;
		}
		...
		if (ci->end >= startAngle && ci->end <= endAngle)
		{
			...
			OGL_DEBUG("  overlapping end with %x => %x:\n", ci->start, ci->end);
			crange = ci->next;
			if (!crange)
			{
				ci->end = endAngle;
				OGL_DEBUG("    no next, adjusting end (now %x => %x)\n", ci->start, ci->end);
				return;
			}
			else
			{
				if (/*crange->start >= startAngle && */crange->start <= endAngle)
				{
					...
					// range AND crange.
					ci->end = crange->end;
					OGL_DEBUG("    merging with the next (%x => %x)\n", crange->start, crange->end);
					C_RemoveRange(crange);
					return;
				}
				else
				{
					...
					ci->end = endAngle;
					OGL_DEBUG("    not merger w/next, ci is now %x => %x\n", ci->start, ci->end);
					return;
				}
			}
		}
	...
	if (!crange)
	{
		OGL_DEBUG("    no suitable new spot, adding to head\n");
	...
	OGL_DEBUG("  spot found, adding after %x => %x\n", crange->start, crange->end);
...
void C_Ranger(void)
{
	...
	for (ci = cliphead; ci; ci = ci->next)
	{
		...
		if (ci->prev)
		{
			...
			I_Error("prev == null, this isn't cliphead.\n");
		...
		OGL_DEBUG("  %p: %04x => %04x ", ci, ci->start, ci->end);
		if (ci->prev)
...
void C_SafeAddRange(binangle startAngle, binangle endAngle)
{
	...
	OGL_DEBUG("Adding range %x => %x...\n", startAngle, endAngle);

	// Check if the range is valid.
	if (!angLen || angLen > BANG_180)
		return;

	// The range may still wrap around.
	if ((int)startAngle+(int)angLen > BANG_MAX)
	{
		OGL_DEBUG("    adding in two parts\n");
...
static int C_SafeCheckRange(binangle startAngle, binangle endAngle)
{
	...
	if ((int)startAngle + (int)angLen > BANG_MAX)
	{
		...
		return (C_IsRangeVisible(startAngle, BANG_MAX) || C_IsRangeVisible(0, endAngle));
	}
...
//	clipnode_t	*cnode = NULL;
	...
	for (i = 0; i < ssec->numedgeverts - 1; i++)
	{
		...
		/*
		if (end == ssec->numedgeverts)
			end = 0;	// Back to beginning.
		*/
